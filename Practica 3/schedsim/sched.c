...
//invoca a la funcion task_tick del planificador en uso.
...
//balanceo de carga -> uso de la funcion steal task de cada planificador
 ...
 * */
/* FUNCION PRINCIPAL
 * ES UN BUCLE CONTINUO EN EL QUE VA LLAMANDO A LAS DISTINTAS FUNCIONES QUE LLAMARAN A LAS FUNCIONES DE CADA PLANIFICADOR
 *
 * 	1- scheduler_tick() -> task_tick de cada planificador: administracion de ticks (si el sistema los utiliza)
 * 	2- process_sched_events() -> lleva a cabo los elementos pendientes de la cpu (un evento es la entrada de una nueva tarea en el sistema o
 * 									una tarea saliendo de su situación de bloqueo) si hay un nuevo evento -> enqueue_task de cada planificador:
 * 									encola la tarea correspondiente
 *
 *
 * 	3- load_balance() ->comprobará si es necesariorealizar un equilibrado de carga. lO hace automatico, solo hay que controlar la sincronizacion
 *
 * 	4- schedule() -> en caso de que sea necesario tomar una decision de planificaCion, es decir, si scheduler_tick() marco la cola de ejecucion para
 * 					 planificacion , procede a escoger la tarea que pasara a ocupar la cpu, entonces llama a pick_next_task() que devolvera la tarea
 * 					 escogida segun el criterio de planificaion
 *
 *	5- synchronize_cpus() -> sincronizacion de cpus
 *
 */
