...
static int my_open(const char *path, struct fuse_file_info *fi)
{
    ...
    if(strcmp(path, "/.myfs_sysinfo") == 0) {
        	...
        	fprintf(stderr,"ç*+++++++++çç+se muestra el archivo :: %i\n",fi->fh);
...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    //buffer que se va a escribir -> cada buffer es un bloque para escribit
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
    ...
    if(myFileSystem.nodes[idxNodoI] == NULL)
        myFileSystem.nodes[idxNodoI] = (NodeStruct *) malloc(sizeof(NodeStruct));
...
static int my_unlink (const char *path)
{
    ...
    //find file on directory
    if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) == -1)
        return -ENOENT;

    //resize to 0 bytes -> will update node.numBlocks,node.fileSize,node.blocks[] but dont remove direct pointer (not necessary)
    if(resizeNode(myFileSystem.directory.files[idxDir].nodeIdx,0) < 0) //actualiza bitmap y el superbloque
        return -EIO;

    //struct node to modify and save
    NodeStruct *node = NULL;
    node = (NodeStruct *) malloc(sizeof(NodeStruct));

    //obtains the i-node associated to the file
    if(readNode(&myFileSystem,myFileSystem.directory.files[idxDir].nodeIdx,node) == -1)
       return -EIO;

    sync();

//     fprintf(stderr,"********el nodo vinculado al archivo a eliminar es: %i\n",myFileSystem.directory.files[idxDir].nodeIdx);

    //modify the node as free
    node->freeNode = 1;
    node->modificationTime = time(0);
    ...
    //El sistema cuando busca un i-nodo libre busca en el array "NodeStruct* nodes[MAX_NODES]" un nodo == NULL.(linea 50, myFs.c),
    //no comprueba si el atrbuto del inodo "freeNode" es true o no.

    //La unica vez que el sistema comprueba el atributo "freeNode" del i-nodo es a la hora de inicializar
    //el sistema en "initializeNodes" (linea 80, myFs.c), por lo tanto si queremos "liberar" el inodo, debemos asignarle el valor NULL

    //the system only recognizes a node as free if its NULL
    myFileSystem.nodes[myFileSystem.directory.files[idxDir].nodeIdx] = NULL;

    updateNode(&myFileSystem,myFileSystem.directory.files[idxDir].nodeIdx,node);
    updateDirectory(&myFileSystem);
    //superBlock and bitmap  were updated on resizeNode
    free(node);
...
static int my_read (const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	if(fi->fh == -1){
		...
		/*
		 * No funciona, no muestra la cadena de caracteres solo somos capaces si la depuramos
		 *
		 *
		 *
		 */

		dest += sprintf(dest, "Fecha de creacion: %t\n",time(NULL));
		dest += sprintf(dest, "Tam Super Bloque: %i\n",sizeof(&myFileSystem.superBlock));
		dest += sprintf(dest, "Tam Directorio: %i\n",sizeof(&myFileSystem.directory));
		dest += sprintf(dest, "Tam Nodo-i: %t\n",sizeof(&myFileSystem.nodes));
		dest += sprintf(dest, "Tam Disco (bloques): %i\n",sizeof(&myFileSystem.superBlock.diskSizeInBlocks));
		dest += sprintf(dest, "Num bloque libres: %i\n",sizeof(&myFileSystem.superBlock.numOfFreeBlocks));
		dest += sprintf(dest, "Tamaño del bloque: %i\n",sizeof(&myFileSystem.superBlock.numOfFreeBlocks));
		dest += sprintf(dest, "Max tam nombre de archivo: %i\n",sizeof(&myFileSystem.superBlock.maxLenFileName));
		dest += sprintf(dest, "Max bloques por archivo: %i\n",sizeof(&myFileSystem.superBlock.maxBlocksPerFile));
		dest += sprintf(dest, "Número de archivos: %i\n",sizeof(&myFileSystem.directory.numFiles));
    ...
    //structura del handler
    NodeStruct *node = myFileSystem.nodes[fi->fh];

    int bytes2Read = size;

    while(bytes2Read){
    	int currentBlock, offBlock = offset % BLOCK_SIZE_BYTES;
